  ...
  if vRP.hasPermission({user_id,cfg.permission}) then
    ...
    if #cops >= cfg.cops then
	  if banks[robb] then
		  ...
		  TriggerClientEvent("pNotify:SendNotification", -1,{text = "Butiksrøveri ved "..bank.nameofbank.."!", type = "warning", queue = "global", timeout = 5000, layout = "bottomCenter",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      ...
      TriggerClientEvent("pNotify:SendNotification", player,{text = "Der skal være " ..cfg.cops.." betjente på, for at røve banken", type = "info", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
    end
  end
end)
...
loop = function()
  local cops = vRP.getUsersByPermission({cfg.permission})
   TriggerClientEvent('vrp_holdup:circlecolor',-1,#cops)
  SetTimeout(5000, loop)
end
SetTimeout(5000, loop)